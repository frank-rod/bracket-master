...
from sqlalchemy.sql.sqltypes import BigInteger, Boolean, DateTime, Enum, Float, JSON, Text
...
# Nuevas tablas para árbitros y scheduling mejorado
referees = Table(
    "referees",
    metadata,
    Column("id", BigInteger, primary_key=True, index=True),
    ...
    Column("certification_level", String, nullable=True),
    Column("created", DateTimeTZ, nullable=False, server_default=func.now()),
    Column("active", Boolean, nullable=False, server_default="t"),
    Column("notes", Text, nullable=True),
)

referees_x_tournaments = Table(
    "referees_x_tournaments",
    metadata,
    Column("id", BigInteger, primary_key=True, index=True),
    Column("referee_id", BigInteger, ForeignKey("referees.id", ondelete="CASCADE"), nullable=False),
    Column("tournament_id", BigInteger, ForeignKey("tournaments.id", ondelete="CASCADE"), nullable=False),
    Column("available_from", DateTimeTZ, nullable=False),
    Column("available_to", DateTimeTZ, nullable=False),
    Column("max_matches_per_day", Integer, nullable=False, server_default="5"),
    Column("preferred_courts", JSON, nullable=True),  # Lista de IDs de canchas preferidas
    Column("notes", Text, nullable=True),
    UniqueConstraint("referee_id", "tournament_id", name="unique_referee_tournament"),
)

referees_x_matches = Table(
    "referees_x_matches",
    metadata,
    Column("id", BigInteger, primary_key=True, index=True),
    Column("referee_id", BigInteger, ForeignKey("referees.id", ondelete="CASCADE"), nullable=False),
    Column("match_id", BigInteger, ForeignKey("matches.id", ondelete="CASCADE"), nullable=False),
    Column("role", String, nullable=False, server_default="'main'"),  # main, assistant, line_judge
    Column("confirmed", Boolean, nullable=False, server_default="f"),
    Column("notes", Text, nullable=True),
    UniqueConstraint("referee_id", "match_id", "role", name="unique_referee_match_role"),
)

# Tabla para slots de tiempo disponibles
time_slots = Table(
    "time_slots",
    metadata,
    Column("id", BigInteger, primary_key=True, index=True),
    Column("tournament_id", BigInteger, ForeignKey("tournaments.id", ondelete="CASCADE"), nullable=False),
    Column("court_id", BigInteger, ForeignKey("courts.id", ondelete="CASCADE"), nullable=False),
    Column("start_time", DateTimeTZ, nullable=False),
    Column("end_time", DateTimeTZ, nullable=False),
    Column("is_available", Boolean, nullable=False, server_default="t"),
    Column("match_id", BigInteger, ForeignKey("matches.id", ondelete="SET NULL"), nullable=True),
    UniqueConstraint("court_id", "start_time", "end_time", name="unique_court_time_slot"),
)
